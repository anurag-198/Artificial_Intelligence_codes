...
double getdis(int stx, int sty, int enx, int eny) {
	...
	while (!pq.empty()) {
		...
		if (((j + 1 < m) && (i + 1 < n)) && (board[i + 1][j + 1] == 0) && (((board[i][j+1] == 0) && (board[i+1][j] == 0))) && (mp1.count(make_pair(i + 1, j+1)) == 0)) {
		...
		if (((j - 1 >= 0) && (i + 1 < n)) && (board[i + 1][j - 1] == 0) && (((board[i][j-1] == 0) && (board[i+1][j] == 0))) && (mp1.count(make_pair(i + 1, j-1)) == 0)) {
			if (mp.count(make_pair(i + 1, j - 1)) == 0) {
				pq.push(make_pair(ex.first + rt2, make_pair(i + 1, j-1)));
				mp.insert(make_pair(make_pair(i + 1,j - 1),ex.first + rt2));
			}
		...
		if (((j - 1 >= 0) && (i - 1 >= 0)) && (board[i - 1][j - 1] == 0) && (((board[i][j-1] == 0) && (board[i-1][j] == 0))) && (mp1.count(make_pair(i-1, j-1)) == 0)) {
			if (mp.count(make_pair(i - 1, j - 1)) == 0) {
				pq.push(make_pair(ex.first + rt2, make_pair(i - 1, j-1)));
				mp.insert(make_pair(make_pair(i - 1,j - 1),ex.first + rt2));
			}
		...
		if (((j + 1 < m) && (i - 1 >= 0)) && (board[i - 1][j + 1] == 0) && (((board[i - 1][j] == 0) && (board[i][j + 1] == 0))) && (mp1.count(make_pair(i - 1, j+1)) == 0)) {
			if (mp.count(make_pair(i - 1, j + 1)) == 0) {
				pq.push(make_pair(ex.first + rt2, make_pair(i - 1, j+1)));
				mp.insert(make_pair(make_pair(i - 1,j + 1),ex.first + rt2));
			}
