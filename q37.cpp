...
map <pii, double> mp1;  // closed from top
map <pii, double> mp3; //  closed from bottom
...
double getdis(int stx, int sty, int enx, int eny) {
	...
	while ((!pq.empty()) && (!pq1.empty())) {
		...
		if (mp3.count(ex.second) > 0) {
			cout << ex.first << " " << mp3[ex.second] << endl;
			return 0;
		}
		
		mp1.insert(make_pair(ex.second, ex.first));
		mp.erase(ex.second);
		i = ex.second.first;
		j = ex.second.second;
	...
	/*****************************************************MOVING UP***************************************************/
		...
		if (mp1.count(ex.second) > 0) {
			cout << ex.first << " " << mp1[ex.second] << endl;
			return 0;
		}

		mp3.insert(make_pair(ex.second, ex.first)); // for closed from bottom
