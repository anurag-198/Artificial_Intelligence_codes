	...
	while (!queue.empty()) {
		...
		if ((j - 1) >= 0) {
			...
			if ((loc4 - 1) >= 0) {
				...
				if (mp.count(ex.first) == 0) {
					queue.push_back(make_pair(ex.first,make_pair (make_pair(i, j - 1), make_pair(loc3, loc4 - 1)) ));
			...
			if ((loc3 - 1) >= 0) {
				...
				if (mp.count(ex.first) == 0) {
					queue.push_back(make_pair(ex.first,make_pair (make_pair(i, j - 1), make_pair(loc3 - 1, loc4)) ));
		...
		if ((i - 1) >= 0) {
			...
			if ((loc4 - 1) >= 0) {
				...
				swapi(&ex.first[loc3][loc4], &ex.first[loc3][loc4 - 1]);
				if (mp.count(ex.first) == 0) {
					queue.push_back(make_pair(ex.first,make_pair (make_pair(i - 1, j), make_pair(loc3, loc4 - 1)) ));
			...
			if ((loc3 - 1) >= 0) {
				...
				swapi(&ex.first[loc3][loc4], &ex.first[loc3 - 1][loc4]);
				if (mp.count(ex.first) == 0) {
					queue.push_back(make_pair(ex.first,make_pair (make_pair(i - 1, j), make_pair(loc3 - 1, loc4)) ));
...
int main() {
	...
	while (tc--) {
		...
		printsol(board,loc1, loc2, loc3, loc4, n);
	}
	return 0;
}